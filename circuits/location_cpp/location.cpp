...
void LocationInIndia_3_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void LocationInIndia_3_run(uint ctx_index,Circom_CalcWit* ctx);
Circom_TemplateFunction _functionTable[4] = { 
Num2Bits_0_run,
LessThan_1_run,
GreaterEqThan_2_run,
LocationInIndia_3_run };
...
uint get_total_signal_no() {return 194;}

uint get_number_of_components() {return 11;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 166;}
...
// load src
Fr_shr(&expaux[1],&signalValues[mySignalStart + 41],&lvar[3]); // line circom 32
...
Fr_eq(&expaux[0],&lvar[1],&signalValues[mySignalStart + 41]); // line circom 38
...
// load src
Fr_sub(&expaux[0],&circuitConstants[2],&ctx->signalValues[ctx->componentMemory[mySubcomponents[0]].signalStart + 40]); // line circom 98
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
for (uint i = 0; i < 1; i++){
...
ctx->componentMemory[coffset].templateName = "LocationInIndia";
ctx->componentMemory[coffset].signalStart = soffset;
ctx->componentMemory[coffset].inputCounter = 2;
ctx->componentMemory[coffset].componentName = componentName;
ctx->componentMemory[coffset].idFather = componentFather;
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "lat_ge_min";
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "lat_le_max";
LessThan_1_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
mySubcomponents[aux_create+i] = aux_cmp_num;
csoffset += 45 ;
aux_cmp_num += 2;
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "lon_ge_min";
GreaterEqThan_2_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
mySubcomponents[aux_create+i] = aux_cmp_num;
csoffset += 48 ;
aux_cmp_num += 3;
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "lon_le_max";
LessThan_1_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
mySubcomponents[aux_create+i] = aux_cmp_num;
csoffset += 45 ;
aux_cmp_num += 2;
...
// load src
Fr_mul(&expaux[0],&ctx->signalValues[ctx->componentMemory[mySubcomponents[0]].signalStart + 0],&ctx->signalValues[ctx->componentMemory[mySubcomponents[1]].signalStart + 0]); // line circom 46
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
// load src
Fr_mul(&expaux[0],&ctx->signalValues[ctx->componentMemory[mySubcomponents[2]].signalStart + 0],&ctx->signalValues[ctx->componentMemory[mySubcomponents[3]].signalStart + 0]); // line circom 47
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
PFrElement aux_dest = &signalValues[mySignalStart + 0];
// load src
Fr_mul(&expaux[0],&signalValues[mySignalStart + 5],&signalValues[mySignalStart + 6]); // line circom 49
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
for (uint i = 0; i < 4; i++){
...
LocationInIndia_3_create(1,0,ctx,"main",0);
LocationInIndia_3_run(0,ctx);
}
