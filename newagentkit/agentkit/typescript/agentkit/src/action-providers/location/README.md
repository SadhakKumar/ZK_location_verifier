...
This directory contains the **LocationActionProvider** implementation, which provides actions for location operations.

## Overview

The LocationActionProvider is designed to work with CdpWalletProvider for blockchain interactions. It provides a set of actions that enable [describe the main purpose/functionality].

## Directory Structure

```
location/
├── locationActionProvider.ts       # Main provider implementation
├── locationActionProvider.test.ts  # Provider test suite
...
This provider is specifically designed to work with CdpWalletProvider. Key integration points:
...
2. Implement the action in `locationActionProvider.ts`:
   ```typescript
   @CreateAction({
     name: "new_action",
   ...
   async newAction(
walletProvider: CdpWalletProvider,      args: z.infer<typeof NewActionSchema>
   ): Promise<string> {
